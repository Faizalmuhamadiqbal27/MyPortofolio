  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-y-4">
        <motion.h3 variants={textVariants(1)} className="text-white font-semibold text-[2rem] tracking-wide">
          {name}
        </motion.h3>
        <motion.h4 variants={textVariants(1.1)} className="text-white font-medium text-[1.4rem]">
          {division[0]}
        </motion.h4>
      </div>
      ...
      <div className="flex flex-wrap lg:flex-row flex-col lg:gap-y-0 gap-y-8 gap-x-8">
        {linkList.map(({ title, href }, index) => (
          <Link href={`/#${href}`} key={index} scroll={false} className={`${theme === 'light' ? 'hover:text-light' : 'hover:text-altLight'} font-normal transition-all`}>
      ...
      {/* Footer Copyright */}
      <motion.span variants={textVariants(1.3)} className="text-white font-normal text-[13px]">
        © 2023 Created by Faizal Muhamad Iqbal. All right reserved
      </motion.span>
    </motion.footer>
  );
}
